...
class EventSignatureClassifier(Tool):
    ...
    def classify_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for event_class in self.event_classes:
            ...
            log_prior = math.log((class_count + self.alpha) / 
                                (self.total_samples + self.alpha * len(self.event_classes)))
            
            # Log likelihood: log P(features|class)
            log_likelihood = 0.0
            feature_vocab_size = len(set(f for counts in self.feature_counts.values() 
    ...
    def get_classification_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "class_priors": {cls: count / self.total_samples 
                           for cls, count in self.class_counts.items()} if self.total_samples > 0 else {},
            "feature_vocabulary_size": len(set(f for counts in self.feature_counts.values() 
                                              for f in counts.keys())),
        }
