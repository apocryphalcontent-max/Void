    ...
    def execute(self, context: HookContext, *args, **kwargs) -> List[Any]:
        ...
        for _priority, callback, filter_fn, _ in self._callbacks:
            ...
            if elapsed > self.overhead_budget_ns:
                print(
                    f"Warning: Hook {self.name} exceeded budget: {elapsed}ns > {self.overhead_budget_ns}ns"
                )
...
class VMHooks:
    ...
    def __init__(self):
        ...
        self.on_memory_access = HookPoint(
            "vm.on_memory_access", HookTiming.BEFORE, overhead_budget_ns=50
        )

        # Snapshot hooks
        self.before_snapshot = HookPoint(
            "vm.before_snapshot", HookTiming.BEFORE, overhead_budget_ns=10_000_000
        )
        self.after_snapshot = HookPoint(
            "vm.after_snapshot", HookTiming.AFTER, overhead_budget_ns=10_000_000
        )
    ...
    def get_instruction_hook(self, opcode: str, timing: HookTiming) -> HookPoint:
        ...
        """
        hooks_dict = (
            self.before_instruction if timing == HookTiming.BEFORE else self.after_instruction
        )
...
class KernelHooks:
    ...
    def __init__(self):
        ...
        self.on_memory_allocation = HookPoint(
            "kernel.on_alloc", HookTiming.BEFORE, overhead_budget_ns=500
        )
        self.on_memory_deallocation = HookPoint(
            "kernel.on_dealloc", HookTiming.AFTER, overhead_budget_ns=500
        )
        self.on_page_fault = HookPoint(
            "kernel.on_page_fault", HookTiming.BEFORE, overhead_budget_ns=10_000
        )
        self.on_gc_cycle = HookPoint(
            "kernel.on_gc", HookTiming.AROUND, overhead_budget_ns=1_000_000
        )

        # Scheduler hooks
        self.on_task_schedule = HookPoint(
            "kernel.on_schedule", HookTiming.BEFORE, overhead_budget_ns=1000
        )
        self.on_context_switch = HookPoint(
            "kernel.on_context_switch", HookTiming.AROUND, overhead_budget_ns=500
        )
        self.on_task_complete = HookPoint(
            "kernel.on_task_complete", HookTiming.AFTER, overhead_budget_ns=1000
        )

        # I/O hooks
        self.on_io_request = HookPoint(
            "kernel.on_io_request", HookTiming.BEFORE, overhead_budget_ns=1000
        )
        self.on_device_io = HookPoint(
            "kernel.on_device_io", HookTiming.BEFORE, overhead_budget_ns=1000
        )
    ...
    def get_syscall_hook(self, syscall: str) -> HookPoint:
        ...
        if syscall not in self.syscall_intercept:
            ...
            self.syscall_intercept[syscall] = HookPoint(
                name, HookTiming.BEFORE, overhead_budget_ns=1000
            )
